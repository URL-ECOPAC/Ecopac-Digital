...
| CP-RF01-07 | Un medico no puede aprobar un movimiento de inventario | La aprobacion la rechaza la base. Desde la issue #864 la pestaña "Validacion" tampoco se le ofrece en la web (`pestanasDeInventario()`), pero lo que decide sigue siendo la base | `pruebas/e2e/inventario-validacion.e2e.test.js`, caso 4 |
...
| CP-RF01-12 | Cada rol ve exactamente lo que le toca, tambien en la base (issue #864) | Junta directiva y socio fundador reciben cero filas de jornadas, proyectos, gastos y donaciones, y conservan `existencias` porque dos de sus reportes la leen directo; el medico lee el proyecto de su jornada y no el de al lado, ve el equipo completo de su jornada, y da de alta un medicamento sin poder editarlo despues; el responsable de una jornada la lee aunque no este en su cuadro de turnos | `permisos_por_rol_864.sql` (23); `packages/shared/navegacion.test.js`; los `permisos.test.js` de cada modulo |
| CP-RF01-13 | La invitacion por correo lleva a poner la contrasena (issue #864) | El correo sale a `${WEB_URL}/nueva-contrasena`, no a la raiz; sin WEB_URL sale igual y gana el Site URL, sin romper el alta | `supabase/functions/invitar-usuario/index_test.ts` (8) |
| CP-RF01-14 | Una cuenta desactivada no entra, y la pantalla lo dice (issue #864) | El intento se rechaza con el mensaje generico -- distinguirlo revelaria que el correo existe --; a quien lo desactivan con la sesion abierta si se le dice la razon | `apps/web/src/pages/LoginPage.test.jsx` (5); `perfil_inactivo_sin_privilegios.sql` (12) |

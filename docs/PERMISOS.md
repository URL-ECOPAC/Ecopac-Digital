...
| `jornada_presupuesto_origen` | C R U D      | R                                | —                | —                  | `00134`. Escribir admite tambien `tiene_permiso('jornadas.gestionar')`, igual que actualizar la jornada; su lectura tambien, por el `INSERT ... RETURNING` |
...
| `donacion_detalle` | C R U (`lote_id`) | R                                | —      | —                  | `00083` + `00086`, mismo cambio. El detalle no se corrige, se anula la donacion completa. Desde la `00134` el renglon de una donacion de medicamentos lleva `medicamento_id`, que exige `fn_registrar_donacion` (INVOKER: no cambia quien puede escribir). El unico UPDATE es enlazar el lote que produjo el renglon: GRANT de la columna `lote_id` y nada mas, una sola vez (la politica solo alcanza `lote_id IS NULL`), con un lote del mismo medicamento (trigger), y lo hace quien registra donaciones (`es_administrador() OR tiene_permiso('donaciones.registrar')`). Hasta la `00134` no habia GRANT ni politica y `enlazarLoteConDonacion()` fallaba siempre |
...
| ----------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------- |
| `jornadas.gestionar`          | administrador                                  | **Si** — INSERT y UPDATE de `jornadas` (`00039`); escritura y lectura de `jornada_presupuesto_origen` (`00134`) |

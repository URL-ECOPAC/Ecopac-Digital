...
| 2 | `condiciones_cronicas`: `crearCondicionCatalogo()`, `actualizarCondicionCatalogo()` y `useCatalogoCondiciones` (issue #641) escriben el catalogo, pero la tabla no tiene GRANT ni politica de escritura (`PERMISOS.md` dice "nadie") y ninguna pantalla usa el hook. | **Resuelto en la #850** (`00140`): lo mantiene el personal que atiende. `GRANT INSERT, UPDATE`, politica de INSERT para administrador, medico y voluntario general, politica de UPDATE solo para administrador, e indice unico sobre el nombre normalizado. El hook `useCatalogoCondiciones` deja de estar huerfano: lo monta `/pacientes/condiciones` en web, y la ficha del paciente -web y movil- da de alta en linea. Pruebas en `escritura_catalogo_condiciones.sql` y `pruebas/e2e/catalogo-condiciones.e2e.test.js`. `PERMISOS.md` actualizado. |

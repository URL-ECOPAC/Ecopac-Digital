...
// 00087, SQL): esta funcion es un envoltorio delgado que solo la invoca y reporta el resultado.
//
...
// })` que no se pudo verificar contra codigo instalado en este entorno (a diferencia de
// expo-crypto en la app movil, donde si hubo un node_modules real contra el que confirmar la
// API). Deno.serve(), createClient() de supabase-js via esm.sh, y Deno.env.get() son las
// primitivas estables y documentadas desde hace años, sin ambiguedad de import ni de contrato:
// se prefiere lo verificable a lo nuevo sin confirmar.
//
...
//    sin GRANT a authenticated/anon, 00087), asi que un JWT valido pero de otro origen -por

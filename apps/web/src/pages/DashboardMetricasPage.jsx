  ...
  <div style={{
    ...
    {meta && (
      <div style={{ height: "6px", backgroundColor: "#e2e8f0", borderRadius: "3px", overflow: "hidden", marginBottom: "6px" }}>
        <div style={{ height: "100%", backgroundColor: color, borderRadius: "3px", width: "70%" }} />
      </div>
    )}
    {meta && (
...
export default function DashboardMetricasPage() {
  ...
  const { cargando, error, indicadores, evolucionMensual, porEspecialidad, topDonantes, estadoJornadas } =
  ...
  if (cargando) return <div style={{ padding: "40px", textAlign: "center", color: "#64748b" }}>Cargando métricas...</div>;
  if (error) return <div style={{ padding: "40px", color: "#dc2626" }}>Error al cargar: {error.message}</div>;
  ...
  return (
    <div style={{ padding: "24px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "24px" }}>
        <TarjetaMetrica etiqueta="Pacientes Atendidos" valor={indicadores?.pacientes || 0} meta="3000" color="#22c55e" />
        <TarjetaMetrica etiqueta="Jornadas Realizadas" valor={indicadores?.jornadas || 0} meta="4" color="#3b82f6" />
        <TarjetaMetrica etiqueta="Voluntarios Activos" valor={indicadores?.voluntarios || 0} meta="10" color="#f59e0b" />
        <TarjetaMetrica etiqueta="Donaciones Recibidas" valor={`Q ${indicadores?.donaciones || 0}`} meta="Q 750,000" color="#ec4899" />
      </div>

      {/* 📈 Gráficas */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "24px" }}>
        {/* Gráfica por mes */}
        <div style={{ backgroundColor: "#fff", borderRadius: "16px", padding: "20px", border: "1px solid #f1f5f9" }}>
          <h3 style={{ fontSize: "16px", fontWeight: "600", color: "#1e293b", margin: "0 0 20px 0" }}>
            Pacientes atendidos por mes
          </h3>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", height: "140px", padding: "0 4px" }}>
            {meses.map((nombre, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "7%" }}>
                  <div style={{ width: "100%", backgroundColor: color, borderRadius: "4px 4px 0 0", height: `${alto}%` }} />
                  <span style={{ fontSize: "11px", color: "#64748b", marginTop: "6px" }}>{nombre}</span>
                </div>
              );
            })}
          </div>
            ...
            {porEspecialidad?.map((esp, i) => (
              <div key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                  <span style={{ fontSize: "14px", fontWeight: "500", color: "#334155" }}>{esp.nombre}</span>
                  <span style={{ fontSize: "14px", fontWeight: "600", color: esp.color }}>{esp.cantidad}</span>
                </div>
                <div style={{ height: "8px", backgroundColor: "#f1f5f9", borderRadius: "4px", overflow: "hidden" }}>
                  <div style={{ width: `${Math.min((esp.cantidad / 1540) * 100, 100)}%`, height: "100%", backgroundColor: esp.color, borderRadius: "4px" }} />
                </div>
              </div>
            ))}
          </div>

...
export function useRegistroSalida({ supabase, onExito }) {
    ...
    const seleccionarLote = (lote) => {
        ...
        if (lote.fecha_vencimiento < hoy) {
            setError(`El lote ${lote.numero_lote} está vencido (Caducó el ${lote.fecha_vencimiento}) y no puede ser seleccionado.`);
            return;
        }
        setError(null);
    ...
    const guardarSalida = async (e) => {
        ...
        if (Number(cantidad) > loteSeleccionado.cantidad_disponible) {
            setError(`La cantidad solicitada supera la existencia disponible del lote (${loteSeleccionado.cantidad_disponible} unidades).`);
            return;
        }

...
export function useAdministracionBodegasProveedores() {
    ...
    const guardarBodega = useCallback(async (bodega) => {
        ...
        if (datos.nombre.length > 100) throw new Error("Nombre demasiado largo (máx. 100 caracteres)");
    ...
    const guardarProveedor = useCallback(async (proveedor) => {
        const datos = {
